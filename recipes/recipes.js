...
function recipeTemplate(recipe) {
  return `
	...
	<div class="content">
	  <p id="dessert">${tagsTemplate(recipe.tags)}</p>
	  <h2>${recipe.name}</h2>
...
function tagsTemplate(tags) {
  return tags.map((tag) => `<p id="dessert">${tag}</p>`).join(" ");
}
...
function renderRecipes(recipeList) {
	let recipeContainer = document.querySelector(".recipe-list"); // Change to a parent container
	if (!recipeContainer) {
	  // If .recipe-list doesn't exist, create it dynamically
	  recipeContainer = document.createElement("div");
	  recipeContainer.className = "recipe-list";
	...
	if (Array.isArray(recipeList)) {
	  ...
	  recipeList.forEach((recipe) => {
		const recipeCard = document.createElement("div"); // Create a new div for each recipe
		recipeCard.className = "recipe-card"; // Assign the recipe-card class
		recipeCard.innerHTML = recipeTemplate(recipe); // Set the HTML for this recipe
		recipeContainer.appendChild(recipeCard); // Append to the parent container
	  });
	} else {
	  // Render a single recipe (e.g., from init)
	  const recipeCard = document.createElement("div");
	  recipeCard.className = "recipe-card";
	  recipeCard.innerHTML = recipeTemplate(recipeList);
  ...
  function sortedRecipes(query) {
	...
	return filterRecipes.sort(compareFn);
  }
  ...
  document.addEventListener('DOMContentLoaded', () => {
	...
	searchButton.addEventListener('click', (e) => {
	  ...
	  let filteredRecipes = sortedRecipes(query);
